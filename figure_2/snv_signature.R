################################################################################
# Fig 2 | COSMIC signature refitting of single-cell SNV spectra
################################################################################
...
mut_mat_snv <- read.table(file.path(input_path, "snv_96_matrix.txt"),
                          header = TRUE, sep = "\t", row.names = 1, check.names = FALSE)
mut_mat_snv <- mut_mat_snv + 0.0001

cell_meta          <- read.csv(file.path(input_path, "snv_burden.csv"))
...
cell_meta <- cell_meta[match(colnames(mut_mat_snv), cell_meta$cell_id), ]
...
sigs2fit <- get_known_signatures(muttype = "snv", incl_poss_artifacts = TRUE)
for (i in seq_len(ncol(sigs2fit))) {
  sigs2fit[, i] <- sigs2fit[, i] / sum(sigs2fit[, i])
}

sig_used   <- rep(FALSE, ncol(sigs2fit))
prev_resid <- sum(mut_mat_snv^2)

next_resids <- sapply(which(!sig_used), function(i) {
  do.call(sum, lapply(seq_len(ncol(mut_mat_snv)), function(j) {
    lsqnonneg(sigs2fit[, i, drop = FALSE], mut_mat_snv[, j])$resid.norm
  }))
})
...
while ((prev_resid - next_resid) / prev_resid > 0.01) {
  next_resids <- sapply(which(!sig_used), function(i) {
    ...
    do.call(sum, lapply(seq_len(ncol(mut_mat_snv)), function(j) {
      lsqnonneg(sigs2fit[, this_sig_used, drop = FALSE], mut_mat_snv[, j])$resid.norm
    }))
  })
  sig_used[!sig_used][which.min(next_resids)] <- TRUE
...
use_cosmic_sigs <- sig_added[seq_len(sum(fit_resid_improvement > 35000, na.rm = TRUE) + 1)]

cell_cosmic_fit <- fit_to_signatures(mut_mat_snv, sigs2fit[, use_cosmic_sigs])
...
################################################################################
# Fig 2c | Signature contribution per cell
################################################################################
...
ggsave(file.path(output_path, "fig2c_ssnv_mutation_spectrum.pdf"), plot = p_spectrum,
       width = 24, height = 5)

################################################################################
# Fig 2d | Signature contribution versus age
################################################################################

age_breaks  <- c(0, 20, 40, 60, 80, 100)
panel_order <- c(1, 3, 4, 9, 6, 8, 7, 10)

sbs_plots <- lapply(unique(contribution_long$variable)[10:1], function(sbs) {
...
ggsave(file.path(output_path, "fig2d_cosmic_age_burden.pdf"),
       plot = marrangeGrob(sbs_plots[panel_order], nrow = 1, ncol = 8, top = ""),
       width = 40, height = 5)

################################################################################
# Fig 2e | Excess signature burden relative to the granule neuron trajectory
################################################################################

sbs_residual_plots <- lapply(unique(contribution_long$variable)[10:1], function(sbs) {
  ...
  ggplot(data_plot, aes(x = celltype, y = gc_resid)) +
    ...
    labs(x = NULL, y = paste0(sbs, " Excess sSNVs per cell")) +
    theme_minimal() +
    theme(panel.grid      = element_blank(),
          panel.border    = element_blank(),
          axis.line       = element_line(color = "black"),
...
ggsave(file.path(output_path, "fig2e_cosmic_burden_residual.pdf"),
       plot = marrangeGrob(sbs_residual_plots[panel_order], nrow = 1, ncol = 8, top = ""),
       width = 40, height = 5)
